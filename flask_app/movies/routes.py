...
def get_b64_img(username):
    ...
    if user and user.profile_pic and user.profile_pic.get():
        print("✅ Found profile pic for", username)
        bytes_im = io.BytesIO(user.profile_pic.read())
        image = base64.b64encode(bytes_im.getvalue()).decode()
        return image
    print("❌ No profile pic for", username)
    return None
...
def movie_detail(movie_id):
    ...
    return render_template(
        "movie_detail.html", form=form, movie=result, reviews=reviews, get_b64_img=get_b64_img
    )
...
def user_detail(username):
    ...
    return render_template("user_detail.html", reviews=reviews, img=img, error=None)